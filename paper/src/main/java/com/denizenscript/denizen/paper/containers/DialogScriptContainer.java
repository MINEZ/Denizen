...
public class DialogScriptContainer extends ScriptContainer {
    ...
    // Requires a Paper server. Note that dialogs are a client-facing UI feature, unrelated to NPC speech.
    //
    ...
    // This executes a Denizen queue when opening the dialog, allowing you to dynamically construct
    // and determine maps for 'inputs', 'bodies', or 'buttons'.
    // Any statically defined elements in the dialog container will be cleanly merged with the generated maps.
    // Note: you can naturally use click-time tags like '<context.button_id>' inside these dynamic scripts,
    ...
    //             width: 15
    //             script:
    //             - narrate "Clicked button <context.button_id>"
    ...
    // - tooltip: <text>
    // - width: <number>
    // - condition: <tag> (the button is only included when this returns 'true')
    ...
    public DialogScriptHelper.InputType parseInput(String type) {
        return switch (CoreUtilities.toLowerCase(type)) {
            case "text" -> DialogScriptHelper.InputType.TEXT;
            case "single" -> DialogScriptHelper.InputType.SINGLE;
            case "boolean", "bool" -> DialogScriptHelper.InputType.BOOLEAN;
            case "number" -> DialogScriptHelper.InputType.NUMBER;
            default -> {
    ...
    public Dialog getDialogFrom(TagContext context, PlayerCommonConnection connection) {
        ...
        final DialogScriptHelper.DialogData dialogData = new DialogScriptHelper.DialogData();
        ...
        return Dialog.create(builder -> builder.empty().base(dialogBase).type(dialogType));
    }

    public void showTo(PlayerCommonConnection connection, TagContext context) {
        if (connection == null) {
            Debug.echoError("Cannot show dialog script '" + getName() + "': no valid connection.");
            return;
        }
        Dialog dialog = getDialogFrom(context, connection);
        if (dialog == null) {
            Debug.echoError("Failed to show dialog script '" + getName() + "'.");
            return;
        }
    ...
    public DialogType getDialogType(TagContext context, DialogScriptHelper.DialogData dialogData, PlayerCommonConnection connection) {
        ...
        return switch (CoreUtilities.toLowerCase(type)) {
            case "confirm" -> {
                ...
                if (yes == null || no == null) {
                    Debug.echoError("Dialog script '" + getName() + "' of type 'confirm' requires both a 'yes' and a 'no' button.");
                    yield null;
                }
            ...
            case "list" -> {
                ...
                for (String id : getStringList("dialogs")) {
                    DialogScriptContainer container = ScriptRegistry.getScriptContainerAs(id, DialogScriptContainer.class);
                    if (container == null) {
            ...
            default -> {
                Debug.echoError("Dialog script '" + getName() + "' has an unknown dialog type: " + type);
                yield null;
            }
        };
    }

    /** 退出按钮按文档写在 'base' 下，同时兼容写在容器根部的旧写法。 */
    ...
    public void bodies(DialogBase.Builder baseBuilder, YamlConfiguration bodiesSection, TagContext context) {
        ...
        for (StringHolder sh : bodiesSection.getKeys(false)) {
            ...
            if (objectSection == null || !checkCondition(objectSection, context)) {
                continue;
            }
            if (!objectSection.contains("type")) {
                Debug.echoError("Dialog script '" + getName() + "' has a body without a specified type!");
                continue;
            }
            ...
            switch (type) {
                ...
                case "item" -> {
                    if (!objectSection.contains("item")) {
                        Debug.echoError("Dialog script '" + getName() + "' has a body without a specified item!");
                        continue;
                    }
                    String raw = getString(objectSection, "item", context);
                    ItemTag itemTag = ItemTag.valueOf(raw, context);
                    if (itemTag == null) {
                        Debug.echoError("'" + raw + "' is not a valid ItemTag.");
                        continue;
                    }
                ...
                default -> Debug.echoError("Dialog script '" + getName() + "' has a body with unknown type: " + type);
            }
        }
        baseBuilder.body(bodies);
    }

    public void inputs(DialogBase.Builder baseBuilder, DialogScriptHelper.DialogData dialogData, TagContext context) {
        YamlConfiguration inputsSection = dialogData.configurationMap.get("inputs");
        if (inputsSection == null) {
            return;
        }
        Map<String, DialogScriptHelper.InputType> inputTypeMap = new HashMap<>();
        ...
        for (StringHolder sh : inputsSection.getKeys(false)) {
            ...
            if (!objectSection.contains("type")) {
                Debug.echoError("Dialog script '" + getName() + "' has an input without a specified type!");
                continue;
            }
            ...
            DialogScriptHelper.InputType type = parseInput(objectSection.getString("type"));
            if (type == null) {
                continue;
            }
    ...
    public PlainMessageDialogBody createPlainMessageDialogBody(YamlConfiguration section, TagContext context) {
        Component message = getComponent(section, "message", context);
        if (message == null) {
            Debug.echoError("Dialog script '" + getName() + "' has a body without a specified message!");
            return null;
        }
    ...
    /** 按钮点击回传的 key，形如 'denizen:script_name/buttons.submit'，点击时据此反查脚本段。 */
    @SuppressWarnings("PatternValidation")
    public Key buttonKey(String buttonPath) {
    ...
    public List<ActionButton> createActionButtons(TagContext context, YamlConfiguration buttonsSection) {
    ...
    public ActionButton createActionButton(String path, YamlConfiguration section, TagContext context) {
        ...
        String type = CoreUtilities.toUpperCase(getString(section, "type", context, "SCRIPT"));
        switch (type) {
            case "SCRIPT" -> {
            ...
            default -> Debug.echoError("Dialog script '" + getName() + "' has an unknown button type '" + type + "' at '" + path + "'.");
        }
        actionButton.action(action);
        return actionButton.build();
    }

    public DialogInput createTextInput(String key, Component label, YamlConfiguration config, TagContext context) {
    ...
    public DialogInput createBooleanInput(String key, Component label, YamlConfiguration config, TagContext context) {
    ...
    public DialogInput createNumberRangeInput(String key, Component label, YamlConfiguration config, TagContext context) {
    ...
    public DialogInput createSingleOptionInput(String key, Component label, YamlConfiguration config, TagContext context) {
        ...
        for (StringHolder optionKey : objectKeys) {
            ...
            Component display = optionSection.contains("display") ? getComponent(optionSection, "display", context) : null;
    ...
    public boolean checkCondition(YamlConfiguration config, TagContext context) {
        if (!config.contains("condition")) {
            return true;
        }
        return CoreUtilities.equalsIgnoreCase(TagManager.tag(config.getString("condition"), context), "true");
    }

    public Component getComponent(YamlConfiguration config, String path, TagContext context) {
        if (!config.contains(path)) {
            return null;
        }
        return PaperModule.parseFormattedText(TagManager.tag(config.getString(path), context), ChatColor.WHITE);
    }

    public Float getFloat(YamlConfiguration config, String path, TagContext context) {
        if (!config.contains(path)) {
            return null;
        }
        String text = TagManager.tag(config.getString(path), context);
        if (ArgumentHelper.matchesDouble(text)) {
            return Float.parseFloat(text);
        }
        Debug.echoError("Dialog script '" + getName() + "' path '" + path + "' has an invalid decimal number: " + text);
        return null;
    }

    public Integer getInt(YamlConfiguration config, String path, TagContext context) {
        if (!config.contains(path)) {
            return null;
        }
        String text = TagManager.tag(config.getString(path), context);
        ...
        Debug.echoError("Dialog script '" + getName() + "' path '" + path + "' has an invalid integer: " + text);
        return null;
    }

    public Boolean getBool(YamlConfiguration config, String path, TagContext context, Boolean def) {
        if (!config.contains(path)) {
            return def;
        }
        return CoreUtilities.equalsIgnoreCase(TagManager.tag(config.getString(path), context), "true");
    }

    public String getString(YamlConfiguration config, String path, TagContext context, String def) {
        if (!config.contains(path)) {
            return def;
        }
    ...
    public String getString(YamlConfiguration config, String path, TagContext context) {
        return getString(config, path, context, null);
    }

    /** 按钮脚本段并不在容器的固定路径下，因此需要按任意配置段构建脚本条目。 */
    public List<ScriptEntry> getEntries(YamlConfiguration section, ScriptEntryData data, String path) {
        List<Object> stringEntries = section.getList(path == null ? "script" : path);
        ...
        return cleanDup(data, new ScriptEntrySet(entries));
    }
}
